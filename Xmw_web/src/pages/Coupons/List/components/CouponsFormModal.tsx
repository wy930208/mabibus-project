...
const CouponsFormModal: FC<Props> = (props) => {
  ...
  return (
    ...
    >
      ...
      {formValue?.coupon_type === 'times' && <ProFormDigit label="使用次数" name="times" />}
      <ProFormRadio.Group
        label="过期类型"
        name="expire_type"
        options={[
      ...
      />
      {formValue?.expire_type === 'fix' && (
        <ProFormDateRangePicker label="有效日期" name="expireTimeRange" />
      )}
      {formValue?.expire_type === 'flex' && <ProFormDigit label="有效天数" name="expireDay" />}
      {/* <ProFormSelect
